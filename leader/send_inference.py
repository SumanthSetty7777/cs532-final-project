...
# how many batches a worker can handle at a time
BACKPRESSURE_CONSTANT = 10

# Find the least busy worker helper function
...
# Main function to handle sending inference
async def send_inference(data, arr, lock):
    ...
    if worker == -3:
        return -3
    # get the data ready to be send as json
    load = [obj.to_dict() for obj in arr]

    # mark this worker as working on a task
    async with lock:
    ...
    # attempt to send message to worker
    try:
        ...
        return -1
    # mark the worker as no longer working on this data
    async with lock:
        for i in range(len(data["workers"])):
            if worker.id == data["workers"][i].id:
                data["workers"][i].cur_input_count -= 1
